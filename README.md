...
Full details of the API are available [on the docker hub page](https://hub.docker.com/r/sbgtechtest/api/)
...
2. Instead of showing all events in one list on the overview page, group them by their `linkedEventTypeName` property. A missing value should cause the grouping to fall back to the `typeName` property.
    1. On the event page, where possible use the `linkedEventTypeName` to highlight the competition the event belongs to.
...
4. Support "deep linking" of events (i.e. enable a user to browse directly to an event page instead of requiring them to navigate from the application homepage)
